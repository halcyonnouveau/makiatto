...
#[tokio::test]
async fn test_health_monitor_detects_failure() -> Result<()> {
    let mut context = ContainerContext::new()?;
    ...
    let insert_peers_sql = format!(
        r"INSERT INTO peers (name, wg_public_key, wg_address, latitude, longitude, ipv4, ipv6, is_nameserver, fs_port) VALUES ('{}', 'pubkey-d2', '10.0.0.20', 51.5074, -0.1278, '127.0.0.1', NULL, 1, {})",
        daemon2_id, daemon2_ports.http
    );

    util::execute_transactions(&daemon1, &[insert_peers_sql]).await?;
    ...
    assert!(
        ...
        "DNS should return results before stopping daemon2"
    );

    let _ = daemon2.stop().await;

    tokio::time::sleep(Duration::from_secs(8)).await;

    let unhealthy_nodes =
        util::query_database(&daemon1, "SELECT node_name FROM unhealthy_nodes;").await?;

    assert!(
        unhealthy_nodes.contains(&*daemon2_id),
        "daemon2 should be marked unhealthy after stopping, got: {unhealthy_nodes}"
    );
    ...
    panic!("DNS should return empty after all peers are marked unhealthy");
}
